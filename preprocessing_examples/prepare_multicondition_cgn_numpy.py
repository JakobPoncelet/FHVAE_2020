...
"""
add noise to CGN train+dev at several SNRs

This script expects you already have created
 - a clean dataset with prepare_cgn_numpy.py
 - a noisy test set with prepare_noisy_cgn_numpy.py
...
/esat/spchdisk/scratch/jponcele/fhvae_jakob/datasets/cgn_np_fbank_afgklno_noisy
/esat/spchdisk/scratch/jponcele/fhvae_jakob/datasets/cgn_np_fbank_afgklno_multicond
...
parser.add_argument("cgn_exp_dir", type=str,
                    help="CGN dir with noisy test set")
...
for set in ["train", "dev"]:
    ...
    wavdict = {}
    rev_wavdict = {}
    with open(os.path.join(args.cgn_exp_dir, set, "wav.scp"), "r") as fd:
        line = fd.readline()
        while line:
    ...
    for wav in noisy_wavs:
        ...
        nspec[set][noise][SNR].append(wavdict[wav])
    ...
    for noise in nspec[set]:
        for SNR in nspec[set][noise]:
            ...
            for wav in wavlist:
                ...
                cmdstring = "sox %s --bits 16 --encoding signed-integer --endian little %s" % (rev_wavdict[wav], out)
    ...
    clean_wav_dict = {}
    with open(os.path.join(args.cgn_exp_dir, set, "wav.scp"), 'r') as rd:
        line = rd.readline()
        while line:
            fname = line.split(" ")[0]
            loc = line.rstrip().split(" ")[1]
            #clean_wav_dict[fname] = loc
            clean_wav_dict[loc] = fname
            line = rd.readline()
    ...
    with open(os.path.join(args.out_dir, set, "wav.scp"), "w+") as td:
        for wav in clean_wavs:
            ...
            td.write(clean_wav_dict[wav]+' '+wav+"\n")
        for fname, loc in scp_dict.items():
            td.write(fname + " " + loc + "\n")

# link to noisy test set
os.symlink(os.path.join(args.cgn_exp_dir, "test"), os.path.join(args.out_dir, "test"))
