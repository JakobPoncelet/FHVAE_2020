...
clear all
%% SETUP
OUTdir='/esat/spchtemp/scratch/jponcele/nbest_augmented/';
add_filter=false;
add_noise=false;
min_SNR=5;  %0 or 5
max_SNR=20;  %15 or 20
%%
% NBEST files
CGNdir='/users/spraak/spchdata/nbest/data/dry-run/vl/';
CGNfiles=dir([CGNdir 'nbest-dev-2008-bn-vl_*.wav']);
...
for k=1:length(CGNfiles)
    ...
    if add_noise
        ...
        while length(noise)<length(sam)
            ...
            % Take random 3 minute (max) sample
            iStart=randi(max(length(NoiseSam)-16000*180,0)+1);
            NoiseSam=NoiseSam(iStart:min(length(NoiseSam),iStart+16000*180-1));
