...
def create_model(conf):
    ...
    if conf['model'] == 'LSTM_attention':
        ...
        model = RegFHVAE_bidirectional_original(z1_dim=conf['z1_dim'], z2_dim=conf['z2_dim'], z1_rhus=conf['z1_rhus'], z2_rhus=conf['z2_rhus'], x_rhus=conf['x_rhus'], nmu2=conf['nmu2'], z1_nlabs=conf['b_n'], z2_nlabs=conf['c_n'], mu_nl=None, logvar_nl=None, tr_shape=conf['tr_shape'], bs=conf['batch_size'], alpha_dis_z1=conf['alpha_dis_z1'], alpha_dis_z2=conf['alpha_dis_z2'], alpha_reg_b=conf['alpha_reg_b'], alpha_reg_c=conf['alpha_reg_c'], n_phones=conf['num_phones'], bump_logpmu1=conf['bump_logpmu1'], alpha_advreg_b=conf['alpha_advreg_b'], alpha_advreg_c=conf['alpha_advreg_c'], priors=conf['priors'], num_flow_steps=conf['num_flow_steps'], num_noisy_versions=conf.get('num_noisy_versions', 0), alpha_noise=conf.get('alpha_noise', 0.0))
