...
class RegFHVAE_attention(tf.keras.Model):
    ''' combine the encoder and decoder into an end-to-end model for training
        uses bidirectional LSTMs for encoders and decoder, but the z2 encoder
        is a pyramidal BiLSTM followed by an attention layer '''
    ...
    def __init__(self, z1_dim=32, z2_dim=32, z1_rhus=[256, 256], z2_rhus=[256, 256], x_rhus=[256, 256],
                 ...
                 bs=256, alpha_dis_z1=1.0, alpha_dis_z2=1.0, alpha_reg_b=1.0, alpha_reg_c=1.0, n_phones=62, priors=[0.5, 1.0, 0.5, 1.0], name="autoencoder", **kwargs):
    ...
    def compute_loss(self, x, y, n, bReg, cReg, mu2, mu1, qz2_x, z2_sample, qz1_x, z1_sample, px_z, x_sample, z1_rlogits, z2_rlogits, num_seqs):
        ...
        # total loss
        loss = -1 * tf.reduce_mean(input_tensor= lb + self.alpha_dis_z2 * log_qy_mu2 + self.alpha_dis_z1 * log_qy_mu1 + self.alpha_reg_b * log_b_loss + self.alpha_reg_c * log_c_loss)
