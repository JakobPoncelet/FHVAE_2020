...
class RegFHVAE_attention(tf.keras.Model):
    ...
    def __init__(self, z1_dim=32, z2_dim=32, z1_rhus=[256, 256], z2_rhus=[256, 256], x_rhus=[256, 256],
        ...
        super(RegFHVAE_attention, self).__init__(name=name, **kwargs)
        ...
        self.encoder = Encoder(self.z1_dim, self.z2_dim, self.z1_rhus, self.z2_rhus,
                               self.tr_shape, self.mu_nl, self.logvar_nl, self.num_flow_steps, self.bs)
...
class Encoder(layers.Layer):
    ...
    def __init__(self, z1_dim=32, z2_dim=32, z1_rhus=[256,256], z2_rhus=[256,256],
                 tr_shape=(20,80), mu_nl=None, logvar_nl=None, num_flow_steps=0, bs=256,
                 name="encoder", **kwargs):
        ...
        self.lstm_layer1_z2 = layers.Bidirectional(
            layers.LSTM(self.z2_rhus[0], return_sequences=True, time_major=False), merge_mode='concat')
        self.lstm_layer2_z2 = layers.Bidirectional(
            layers.LSTM(self.z2_rhus[1], return_sequences=True, time_major=False), merge_mode='concat')
        self.attn_fc_v = layers.Dense(2*self.z2_rhus[1], use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros')
        self.attn_fc_q = layers.Dense(2*self.z2_rhus[1], use_bias=True, kernel_initializer='glorot_uniform', bias_initializer='zeros')
        self.attn = layers.Attention(use_scale=True)
        self.query = tf.Variable(tf.random.normal([self.bs, 1, 2*self.z2_rhus[1]], stddev=1.0), trainable=True)

        # RNN specs for z1_pre_encoder
        self.z1_rhus = z1_rhus

        ## Bidirectional LSTMs
        self.lstm_layer1_z1 = layers.Bidirectional(
            layers.LSTM(self.z1_rhus[0], return_sequences=True, return_state=True, time_major=False),
            merge_mode='concat')
